...
class pyStar(object):
    ...
    def before_evolve_loop(self):
        res = self.star.before_evolve_loop(
            False,
            True,
            False,
            self.star.null_binary_controls,
            self.rse.extras_controls,
            0,
            self.inlist,
            "restart_photo",
            True,
    ...
    def single_step(self):
        res = self.star.star_evolve_step(
            self.star_id, self.first_try, self.just_did_backup
        )
    ...
    def single_evolve(self):
        ...
        if res != self.star_def.keep_going:
            if res != self.star_def.terminate:
                ...
                res = self.star.terminate_normal_evolve_loop(
                    self.star_id, 0, False, res, 0
                )
                self.error_check(res)
                self.continue_evolve_loop = False
                return False
    ...
    def get_prof_nz(self, name):
        ...
        for i in range(1, nz + 1):
            output[i] = self.star_lib.star_get_profile_output_by_id(
                self.star_id, name, i
            )
        return output
...
def setinlist():
    ...
    print(s.get_dt())
    s.star.star_set_v_flag(
        s.star_id, True, 0
    )  # Can call any star_lib function that takes id instead of s
    s.single_evolve()  # One step
