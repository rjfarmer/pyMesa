...
if not gf.__version__.startswith("3."):
    raise ValueError(
        f"Unsupported gfort2py version {gf.__version__}, must be greater than 3.0.0"
    )
...
def loadMod(module):
    ...
    try:
        ...
        if os.path.exists(os.path.join(BUILD_DIR, module, "lib", f"lib{module}.a")):
            raise MesaError(
                f"Shared library for {module} not found, but static library exists. Please build with shared libraries enabled."
            )
        pass
